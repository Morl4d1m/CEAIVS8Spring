...
load("C:\Users\Christian Lykke\Documents\Skole\Aalborg Universitet\CEAIVS8\Projekt\Lydfiler\efterKlangLydLab.mat")
...
exportCSV = 1;
exportLatex = 0;
outputFolder = "C:\Users\Christian Lykke\Documents\Skole\Aalborg Universitet\CEAIVS8\Projekt\Lydfiler\impulseResponseMeasurerData\lydLab";
...
isSweep = strcmp(measurementData.Method,'Swept Sine');
isMLS   = strcmp(measurementData.Method,'MLS');

methods  = {'Sweep','MLS'};
%datasets = {irdata_084724(isSweep,:), irdata_084724(isMLS,:)};
%datasets = {irdata_120508(isSweep,:), irdata_120508(isMLS,:)};
datasets = {measurementData(isSweep,:), measurementData(isMLS,:)};
...
for methodIdx = 1:2
    ...
    %% Spatial averages (robust for 1 spk / 1 mic)
    
    avgEDT  = squeeze(mean(EDT , [1 2], 'omitnan'));
    avgRT10 = squeeze(mean(RT10, [1 2], 'omitnan'));
    avgT20  = squeeze(mean(T20 , [1 2], 'omitnan'));
    avgT30  = squeeze(mean(T30 , [1 2], 'omitnan'));
    avgRT60 = squeeze(mean(RT60, [1 2], 'omitnan'));
    avgC50  = squeeze(mean(C50 , [1 2], 'omitnan'));
    avgC80  = squeeze(mean(C80 , [1 2], 'omitnan'));
    avgD50  = squeeze(mean(D50 , [1 2], 'omitnan'));
    avgD80  = squeeze(mean(D80 , [1 2], 'omitnan'));
